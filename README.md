...
> **Última atualização do snapshot: 03/09/2026 12:31** (`2026-09-03T12:31:57-03:00`)
...
### O que mudou desde a geração anterior (01/09/2026 19:29)

- **Nenhuma alteração nos números.** Os 6 conjuntos de setembro/2026 foram reconsultados no Jira nesta geração e a comparação campo a campo (chave, `summary`, `status.name`, `statusCategory`, projeto, `duedate`, `resolutiondate` e `updated`) com os JSONs de `_snap/` deu **idêntico** nos 6: 22 previstos, 2 em atraso acumulado, 25 no lookahead, 0 envios, 0 concluídos, 0 com retrabalho.
- `sent_2026-09`, `resolved_2026-09` e `rework_2026-09` voltaram vazios do Jira — nenhuma entrega registrada nos três primeiros dias de setembro.
- Os dois EPICs em atraso acumulado seguem sendo `EG0286-8` (Estudo topográfico) e `EG0286-7` (Estudo de tráfego), ambos com due date em 31/08/2026 e ainda em andamento.
- A lista de projetos visíveis **foi reconsultada** via `getVisibleJiraProjects` e conferiu com o `_projects_min.json` existente (mesmo SHA-256 do JSON canônico; 19 projetos; tipos de nível Epic `Epic` e `Fluxo de trabalho`), que por isso não precisou ser reescrito.
...
- **Nenhuma requisição para a Atlassian** — percorrendo as três abas, o navegador emitiu 2 requisições no total: o próprio `index.html` e o `chart.js` do CDN. 0 requisições para `atlassian.net` ou qualquer host da Atlassian.
- **Nenhum aviso `[PR03] JQL sem correspondencia no snapshot`** no console — todos os 11 padrões de JQL resolveram.
- Console registra `[PR03] Snapshot estatico carregado - gerado em 2026-09-03T12:31:57-03:00; consultas ao Jira desativadas.`
- **0 erros de JavaScript**, nenhuma mensagem de console em nível `warning`/`error` e nenhum recurso 404. Abas "Agosto 2026 — Encerrado", "Setembro 2026 — Ao vivo" e "Visão Acumulada — Histórico" presentes; nenhuma mensagem "Erro na inicialização"; seletores de projeto com "Todos" + os 19 projetos (40 `option` nos dois seletores).
...
- Observação sobre o ambiente de verificação: o teste roda sobre o **`index.html` publicado, sem qualquer alteração**. A requisição do `chart.js@4.5.0` é respondida com o arquivo real baixado do mesmo CDN (`cdn.jsdelivr.net/npm/chart.js@4.5.0/dist/chart.umd.js`), cujo SHA-384 confere com o `integrity` (SRI) declarado na tag — a validação de SRI do navegador passa. No Vercel a tag carrega do CDN normalmente.
