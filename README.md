...
> **Última atualização do snapshot: 25/09/2026 12:30** (`2026-09-25T12:30:03-03:00`)
...
### O que mudou desde a geração anterior (25/09/2026 11:29)

**Nenhuma mudança no Jira.** As seis consultas de setembro/2026 e a lista de projetos foram refeitas ao vivo e devolveram exatamente os mesmos registros da geração das 11:29 — mesmas chaves, mesma ordem, mesmos status, prazos, datas de resolução e `updated`.

| | Geração anterior (11:29 de 25/09) | Agora (12:30 de 25/09) |
|---|---:|---:|
...
- A conferência foi feita **registro a registro** e por **script**, não por contagem: o `_snap/_verify0925c.py` compara chave, status, categoria, projeto, prazo, data de resolução e `updated` de cada registro ao vivo contra o `_snap/*.json` gravado, e falha com código de saída 1 em qualquer divergência. Nesta geração passou nos 6 conjuntos.
- O `lookahead_2026-09` (out/2026 + nov/2026) foi executado em **duas metades** — outubro (20 registros) e novembro (9) — para não estourar o limite de tokens do MCP. Os dois epics do **EG0274** reagendados na manhã de 25/09 seguem fora de setembro: `EG0274-21` em 29/10 e `EG0274-58` em 30/10.
- A lista de projetos foi **reconsultada ao vivo**: 20 projetos, mesmas chaves de `_projects_min.json` (`CREA`, `EG0232`, `EG0235`, `EG0239`, `EG0240`, `EG0241`, `EG0256`, `EG0257`, `EG0272`, `EG0273`, `EG0274`, `EG0275`, `EG0285`, `EG0286`, `EG0287`, `EG0292`, `EG291`, `G0120`, `G0280`, `PE`), e os tipos de nível Epic inalterados (`Epic`, `Fluxo de trabalho`). O `_projects_min.json` do repositório foi mantido.
...
- Única mensagem de console de toda a sessão: `[PR03] Snapshot estatico carregado - gerado em 2026-09-25T12:30:03-03:00; consultas ao Jira desativadas.`
...
- `window.__SNAPSHOT__.generatedAt` = `2026-09-25T12:30:03-03:00`, 20 projetos, `epicTypeNames` = `Epic`, `Fluxo de trabalho`, e `window.__HISTORY__` com os 5 meses congelados (`2026-04` a `2026-08`) — confirmando que o merge defensivo do `__HISTORY__` preservou os meses do snapshot.
...
| `_snap/_verify0925c.py` | — | Conferência automatizada desta geração (12:30 de 25/09), registro a registro |
| `_snap/_run0925b.py` | — | Carga dos dados de setembro em vigor (reagendamento de `EG0274-58` e `EG0274-21`) |
