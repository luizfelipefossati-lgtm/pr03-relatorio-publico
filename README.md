...
> **Última atualização do snapshot: 03/09/2026 16:30** (`2026-09-03T16:30:05-03:00`)
...
### O que mudou desde a geração anterior (03/09/2026 15:30)

- **Nenhuma alteração nos números.** Os 6 conjuntos de setembro/2026 foram reconsultados no Jira nesta geração e conferiram exatamente com os JSONs de `_snap/` (mesmas chaves, status, due dates e datas de resolução): 22 previstos, 2 em atraso acumulado, 25 no lookahead, 0 envios, 0 concluídos, 0 com retrabalho.
...
- A lista de projetos visíveis **foi reconsultada** nesta geração (`getVisibleJiraProjects`, 19 projetos) e confere chave por chave com o `_projects_min.json` do repositório, que por isso foi reaproveitado sem alteração. Tipos de nível Epic: `Epic` e `Fluxo de trabalho`.
...
- Única mensagem de console de toda a sessão: `[PR03] Snapshot estatico carregado - gerado em 2026-09-03T16:30:05-03:00; consultas ao Jira desativadas.`
- **0 erros de JavaScript**, 0 mensagens em nível `warning`/`error`, nenhum recurso 404 e nenhuma mensagem "Erro na inicialização".
- Abas "Agosto 2026 — Encerrado", "Setembro 2026 — Ao vivo" e "Visão Acumulada — Histórico" presentes; seletores de projeto `sP` e `fP` com 20 `option` cada ("Todos" + os 19 projetos), 80 `option` no total da página contando os seletores de mês/ano da Visão Acumulada.
- `window.__SNAPSHOT__.generatedAt` = `2026-09-03T16:30:05-03:00`, 19 projetos, e `window.__HISTORY__` com os 5 meses congelados (`2026-04` a `2026-08`) — confirmando que o merge defensivo do `__HISTORY__` preservou os meses do snapshot.
...
- Observação sobre o ambiente de verificação: a requisição do `chart.js@4.5.0` foi respondida com o arquivo real baixado do mesmo CDN (`cdn.jsdelivr.net/npm/chart.js@4.5.0/dist/chart.umd.js`), cujo SHA-384 (`iU8HYtnGQ8Cy4zl7gbNMOhsDTTKX02BTXptVP/vqAWIaTfM7isw76iyZCsjL2eVi`) confere com o `integrity` (SRI) declarado na tag. No Vercel a tag carrega do CDN normalmente.
